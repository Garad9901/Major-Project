...
from .untrusted import fence_passages, fence_sql_rows
...
def _format_sql_section(sql_result):
    ...
    """
    return fence_sql_rows(sql_result)
...
def _format_rag_section(rag_chunks):
    ...
    """
    return fence_passages(rag_chunks)
...
def _format_web_section(web_pages):
    if not web_pages:
        return ""
    return fence_passages([_WebAsPassage(p) for p in web_pages])


def _combined_context(rag_chunks, web_pages):
    ...
    """
    parts = [s for s in (_format_rag_section(rag_chunks), _format_web_section(web_pages)) if s]
    return "\n\n".join(parts)
